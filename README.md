...
- **全库搜索**：搜索框输入关键词后**回车**，跨作者搜索角色/系列名（如不记得某角色属于哪个作者）→ 结果按 `作者 › 系列 › 角色` 列出，点击直达该角色图片层。后端进程内缓存目录树索引（无建库），首次搜索触发后台构建（NAS 全库约 ~20s），期间轮询就绪后自动出结果；新下载/移动后顶层目录变化会自动触发索引重建
- **查看器**：静态图手势滑动翻页 / 双指缩放 / 双击放大，动图 canvas 帧播放；键盘 ←/→ 切换
- **下载**：粘贴链接 → 预览 meta+标签 → 手动选择系列/角色 → 后台任务 + 进度轮询；下载视图展示服务端全部任务（含快捷指令 API 触发的），下载中/失败任务可重试，已结束任务可清除。下载前**同作者查重**：作品已在该作者目录（别名角色目录/同系列其他目录/_未分類/Collections）存在时自动跳过不重复落盘，任务标记"已存在"
...
├── .thumbs/        # 缩略图缓存
└── {作者}/          # 归档作品
    ├── Collections/{作品ID}_{标题}/    # 合集/无系列/无正式名称角色（多角色混合/原创/neta）
    ├── {系列}/{角色}/            # 静态图 {id}_p{n}.ext + 动图 {id}.zip/.frames.json/.meta.json（同目录混合）
    ├── {系列}/_未分類/            # 有系列但角色未定（临时待分类）
    └── _未分類/                  # 其余无系列作品（平铺，webapp 未指定系列/角色时兜底）
```
